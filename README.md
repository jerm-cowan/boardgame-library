...
- `/dashboard` — the acting layer: filter your collection and get a data-backed recommendation for what to play today.
...
Each game also carries an `audience: 'family' | 'adults' | 'mixed'` tag (added in Phase 3, since the original data model had no audience/age field) used by the "Pick Today's Game" recommender's "who's playing" input. It's heuristically derived from category + complexity (Family/Party at complexity ≤3 → `family`; Strategy/Abstract at complexity ≥4 → `adults`; everything else → `mixed`).

## Recommendation logic ("Pick Today's Game")
